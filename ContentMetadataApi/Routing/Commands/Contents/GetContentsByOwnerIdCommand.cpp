#include "GetContentsByOwnerIdCommand.hpp"
...
namespace ContentMetadataApi
{
	namespace Routing
	{
		GetContentsByOwnerIdCommand::GetContentsByOwnerIdCommand(
		...
		auto GetContentsByOwnerIdCommand::executeAsync() const -> boost::asio::awaitable<HttpResponse>
		{
			std::cout << "Start executing GetContentsByOwnerIdCommand" << std::endl;
			...
			try
			{
				auto content_entities = co_await m_contents_contract->getContentMetadataByOwnerIdAsync(m_content_id);
			...
			{
				std::cerr << "Get content metadata failed: " << e.what() << std::endl;
				throw;
			}
			...
			{
				std::cerr << "Recieved contents data not valid: " << std::endl;
			...
			std::cout << "Finish executing GetContentsByOwnerIdCommand with success" << std::endl;
