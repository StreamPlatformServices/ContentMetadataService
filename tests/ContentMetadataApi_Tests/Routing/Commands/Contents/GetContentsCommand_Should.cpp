#include "GetContentsCommand_Should.hpp"
...
{
    TEST_F(GetContentsCommand_Should, ReturnOkResponseWithContents)
    {
        ...
        EXPECT_CALL(*m_mock_contract, getAllContentsAsync(::testing::_, ::testing::_))
            .WillOnce(::testing::Invoke([&](int limit, int offset) -> boost::asio::awaitable<std::vector<ContentMetadataCore::Entities::Content>> {
    ...
    TEST_F(GetContentsCommand_Should, ReturnBadRequestWhenQueryParamsAreInvalid)
    {
        m_query_params = { { "limit", "invalid" }, { "offset", "invalid" } };

        std::vector<std::string> validation_errors = { "Query parameter limit invalid.", "Query parameter offset invalid." };
        EXPECT_CALL(*m_mock_visitor_factory, createJsonSerializationVisitor(::testing::_))
            .WillOnce([&validation_errors](std::string& body) {
            return std::make_unique<::testing::NiceMock<SerializationVisitorMock>>();
                });
    ...
    TEST_F(GetContentsCommand_Should, ThrowExceptionWhenFetchingFails)
    {
        EXPECT_CALL(*m_mock_contract, getAllContentsAsync(::testing::_, ::testing::_))
            .WillOnce(::testing::Invoke([](int limit, int offset) -> boost::asio::awaitable<std::vector<ContentMetadataCore::Entities::Content>> {
    ...
    TEST_F(GetContentsCommand_Should, ThrowExceptionWhenValidationFails)
    {
